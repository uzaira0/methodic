...
Place your operator-provided TLS certificates here:

- `chronicle.example.com.crt` - Full certificate chain (cert + intermediate CA)
- `chronicle.example.com.key` - Private key (PEM format)
...
```bash
cat server.crt intermediate.crt > chronicle.example.com.crt
```
...
1. **Verify cert/key match:**
   ```bash
   openssl x509 -noout -modulus -in chronicle.example.com.crt | openssl md5
   openssl rsa -noout -modulus -in chronicle.example.com.key | openssl md5
   # Both MD5 hashes must match
   ```

2. **Verify cert chain is complete:**
   ```bash
   openssl verify -CAfile /etc/pki/tls/certs/ca-bundle.crt chronicle.example.com.crt
   # Should print: chronicle.example.com.crt: OK
   ```

3. **Check cert expiry and SAN:**
   ```bash
   openssl x509 -noout -dates -subject -ext subjectAltName -in chronicle.example.com.crt
   ```
...
4. **Install Traefik configs (requires root/sudo):**
   ...
   sudo cp chronicle.example.com.crt /etc/traefik/certs/
   sudo cp chronicle.example.com.key /etc/traefik/certs/
...
7. **Verify TLS is working:**
   ```bash
   curl -vI https://chronicle.example.com/chronicle
   # Check: TLS 1.2+, correct cert, 200 OK
   ```

## Certificate Renewal

When the operator receives renewed certs, replace the files above and restart Traefik:
```bash
