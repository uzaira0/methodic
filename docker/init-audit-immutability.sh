...
# Apply triggers and revoke permissions from every application role. The
# bootstrap owner remains able to perform governed retention work, but the
# request-path role must never be able to rewrite or delete audit history.
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<EOSQL
    DO \$\$
    BEGIN
