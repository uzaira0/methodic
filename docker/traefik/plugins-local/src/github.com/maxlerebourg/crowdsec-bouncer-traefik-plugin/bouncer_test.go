...
func TestHandleNextServeHTTPInjectsSanitizedClientIP(t *testing.T) {
	...
	bouncer := &Bouncer{
		...
		injectClientIPHeader:  true,
	}
	req := httptest.NewRequest(http.MethodPost, "http://chronicle.test/chronicle/v4/study/s/participant/p/android", nil)
	req.Header.Set("X-Forwarded-For", "198.51.100.99, 127.0.0.1")

	bouncer.handleNextServeHTTP(httptest.NewRecorder(), req, "203.0.113.10")

	if forwardedFor != "203.0.113.10" {
		t.Fatalf("expected the resolved client IP only, got %q", forwardedFor)
	}
}
