...
# Keep the optional password override in the operator shell, never in host child
# environments. PostgreSQL receives it over the Docker stdin channel below.
...
psql_cmd() {
  {
    ...
    /bin/bash -euc '
      ...
      exec psql -h 127.0.0.1 -U "$postgres_user" -d "$postgres_db" \
        -v ON_ERROR_STOP=1 -v study_id="$study_id" -P pager=off "$@"
    ' chronicle-set-android-sensors "$POSTGRES_USER" "$POSTGRES_DB" \
      "$STUDY_ID" "$@"
}
...
psql_cmd -v setting_json="$setting_json" <<'SQL'
UPDATE studies
