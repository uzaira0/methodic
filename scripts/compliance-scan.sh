...
# Validate args before any destructive (prune) work. --retention-cycles must be a positive
# integer: 0 / non-numeric would make `head -n -0` emit EVERY cycle dir and `rm -rf` all of
# the HIPAA audit evidence. --evidence-root must be a non-empty absolute path so the prune
# glob and mkdir never operate on the filesystem root or a relative tree.
case "$RETENTION_CYCLES" in
  ''|*[!0-9]*) echo "--retention-cycles must be a positive integer, got '$RETENTION_CYCLES'" >&2; exit 2 ;;
esac
[ "$RETENTION_CYCLES" -ge 1 ] || { echo "--retention-cycles must be >= 1 (refusing to prune all evidence)" >&2; exit 2; }
case "$EVIDENCE_ROOT" in
  /?*) : ;;
  *) echo "--evidence-root must be a non-empty absolute path, got '$EVIDENCE_ROOT'" >&2; exit 2 ;;
esac
...
# vuln-scan runs on a 6-month cadence; the annual pentest-prep snapshot is 12-monthly.
if [ "$CYCLE" = "pentest-prep" ]; then NEXT_INTERVAL="+12 months"; else NEXT_INTERVAL="+6 months"; fi
NEXT_DATE="$(date -d "${CYCLE_DATE} ${NEXT_INTERVAL}" +%Y-%m-%d 2>/dev/null || echo "unknown")"
...
if command -v grype >/dev/null 2>&1; then
  ...
  grype --config "$ROOT_DIR/.grype.yaml" "dir:$ROOT_DIR" \
    --only-fixed -o "sarif=$EVIDENCE_DIR/grype.sarif" > "$EVIDENCE_DIR/grype.log" 2>&1
  rc=$?; set -e
  ...
  osv-scanner scan source \
    ...
    --format sarif \
    --output-file "$EVIDENCE_DIR/osv.sarif" \
    "$ROOT_DIR" > "$EVIDENCE_DIR/osv.log" 2>&1
  rc=$?; set -e
...
_next_label = "Next annual cycle" if cycle == "pentest-prep" else "Next 6-month cycle"
lines.append(f"**{_next_label}:** {next_date}  ")
